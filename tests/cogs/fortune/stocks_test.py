...
async def test_stock_info_single_stock(ticker, bot, message, text):
    ...
    message.channel.send.assert_called_once_with("DUX (Ducks): $100.00 (USD) per share, -0.99% today; +10.0% over a year")
...
async def test_stock_info_multiple_stocks(ticker, bot, message, text):
    ...
    dux2 = info(symbol="DUX2", close=99, year_change=-0.1)
    ...
    message.channel.send.assert_called_once_with("DUX1 (Ducks): $100.00 (USD) per share, -0.99% today; +10.0% over a year\nDUX2 (Ducks): $100.00 (USD) per share, +1.01% today; -10.0% over a year")
