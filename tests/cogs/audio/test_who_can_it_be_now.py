...
async def test_task_loop_once(ffmpeg, vol, bot_spy, command_text_context, voice_client):
...
@mock.patch("duckbot.cogs.audio.who_can_it_be_now.PCMVolumeTransformer", autospec=True)
@mock.patch("duckbot.cogs.audio.who_can_it_be_now.FFmpegPCMAudio", autospec=True)
async def test_task_loop_repeats(ffmpeg, vol, bot_spy, command_text_context, voice_client):
    command_text_context.voice_client = None
    command_text_context.author.voice.channel.connect.return_value = async_value(voice_client)
...
async def test_connect_to_voice_no_voice(bot, command_context):
    ...
    command_context.send.assert_called_once_with("Music can only be played in a discord server, not a private channel.", delete_after=30)
...
async def test_connect_to_voice_author_not_in_channel(bot, command_context):
    ...
    with pytest.raises(CommandError):
        await clazz.connect_to_voice(command_context)
    command_context.send.assert_called_once_with("Connect to a voice channel so I know where to `!start`.", delete_after=30)
...
@mock.patch("duckbot.cogs.audio.who_can_it_be_now.PCMVolumeTransformer", autospec=True)
@mock.patch("duckbot.cogs.audio.who_can_it_be_now.FFmpegPCMAudio", autospec=True)
async def test_stop_disconnects(ffmpeg, vol, bot, command_context, voice_client):
    clazz = WhoCanItBeNow(bot)
    clazz.streaming = True
...
async def test_stop_not_streaming(bot, command_context):
    ...
    await clazz._WhoCanItBeNow__stop(command_context)
    command_context.send.assert_called_once_with("Brother, no :musical_note: :saxophone: is active.", delete_after=30)
