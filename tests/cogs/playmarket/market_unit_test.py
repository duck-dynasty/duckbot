...
def test_winning_yes_pays_one_coin_per_share(cog):
    ...
    assert cog._payout(holding, "yes") == Decimal(10)


def test_winning_no_pays_one_coin_per_share(cog):
    holding = SimpleNamespace(yes_shares=Decimal(10), no_shares=Decimal(4))
    assert cog._payout(holding, "no") == Decimal(4)


def test_void_pays_half_of_every_share(cog):
    holding = SimpleNamespace(yes_shares=Decimal(10), no_shares=Decimal(4))
    assert cog._payout(holding, "void") == Decimal(7)


def test_void_payout_rounds_toward_the_house(cog):
    holding = SimpleNamespace(yes_shares=Decimal(3), no_shares=Decimal(0))  # 1.5 -> 1.5, but 3/2 stays exact
    assert cog._payout(holding, "void") == Decimal("1.5")


def test_holding_a_bond_moves_coins_from_balance_to_locked(cog):
    player = SimpleNamespace(balance=Decimal(100), locked=Decimal(0))
    cog._hold_bond(player, config.PROPOSE_BOND)
    assert (player.balance, player.locked) == (Decimal(50), Decimal(50))


def test_releasing_a_bond_is_the_inverse_of_holding(cog):
    player = SimpleNamespace(balance=Decimal(50), locked=Decimal(50))
    cog._release_bond(player, Decimal(50))
    assert (player.balance, player.locked) == (Decimal(100), Decimal(0))


# --- rounding & formatting ----------------------------------------------


def test_down_truncates_toward_the_house():
...
@pytest.mark.parametrize("value,shown", [(Decimal("1000"), "1,000"), (Decimal("83.6"), "84"), (Decimal("0"), "0")])
