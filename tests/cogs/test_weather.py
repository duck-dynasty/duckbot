...
async def test_weather_failure(bot, owm, context):
...
@mock.patch("discord.ext.commands.Bot")
@mock.patch("pyowm.OWM")
def test_weather_message(bot, owm):
    clazz = make_weather(bot, owm)
    print(clazz.weather_message(make_city("city"), one_call()))
...
def one_call():
    return OneCall(lat=1, lon=1, timezone="UTC", current=weather(), forecast_daily=[weather()])
...
def weather():
    return pyowmWeather(
        ...
        humidity=0,
        pressure=None,
        temperature={"temp": 3, "min": 0, "max": 10, "feels_like_day": 15, "feels_like_night": -10, "feels_like": 5},
        status="rain",
        detailed_status="super rain",
        weather_code=None,
