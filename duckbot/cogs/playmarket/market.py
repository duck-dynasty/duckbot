...
class PlayMarket(commands.Cog):
    ...
    async def bet(self, context: commands.Context, market_id: int, side: str, amount: int):
        ...
        with self.db.session(Market) as session:
            ...
            await context.send(embed=await self._trade_embed(context, session, market, action))
    ...
    async def _trade_embed(self, context, session, market, action: str) -> Embed:
        """Confirm a bet/sell, showing the new YES % and everyone's holdings in the market."""
        embed = Embed(title=f"Market {market.id} — {market.question}", description=f"{action}\nYES is now {_pct(self._price(market))}.")
