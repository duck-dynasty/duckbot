...
class AnnounceDay(commands.Cog):
    ...
    def get_message(self):
        ...
        message = random.choice(templates + days[day]["templates"]).format(
            today, tomorrow
        )
...
class SpecialDays(holidays.Canada):
    ...
    def _populate(self, year):
        ...
        self[
            datetime.date(year, 5, 7)
        ] = f"Bro Tito Day {self.bot.get_cog('emojis').get_emoji_by_name('tito')}"
        self[datetime.date(year, 6, 1) + rd(weekday=SU(+3))] = "Father's Day"
        self[
            datetime.date(year, 6, 21)
        ] = f"Erin's Birthday. She's {year-1991} years old"
        self[datetime.date(year, 9, 8)] = f"Dan's Birthday. He {year-1989} old"
        self[
            datetime.date(year, 9, 26)
        ...
        self[
            datetime.date(year, 12, 2)
        ] = f"Female Kelly's Birthday. She's {year-1989} years old"
        self[datetime.date(year, 12, 3)] = "DuckBot's Inception Day"
        self[
            datetime.date(year, 12, 5)
        ] = f"Taras' Birthday. He's {year-1989} years old"
